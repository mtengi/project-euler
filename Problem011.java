 ...
 * Written: Jun 8, 2013
 * 
 * Project Euler: Problem011 - Largest product in a grid
 * 
 * 
...
public class Problem011 {
    ...
    public static void main(String[] args) {
        ...
        for (int x = 0; x < grid.length; x++) {
            for (int y = 0; y < grid.length; y++) {
                ...
                // diag2
                if (x < grid.length - consecutive && y >= consecutive - 1) {
                    int prod = 1;
                    for (int i = 0; i < 4; i++) {
                        prod *= grid[x + i][y-i];
                    }
        ...
        for (int i = 0; i < grid.length; i++) {
            largest = crawl(0, i, 1, 0, largest);
            // System.out.println(largest);
            largest = crawl(i, 0, 0, 1, largest);
            largest = crawl(0, i, 1, 1, largest);
            largest = crawl(i, 0, 1, 1, largest);
            largest = crawl(0, i, -1, -1, largest);
            largest = crawl(i, 0, -1, -1, largest);
        }
        System.out.println(largest);
    ...
    public static int crawl(int startX, int startY, int delX, int delY,
            int largest) {
        int prod = getConsecutive(startX, startY, delX, delY);
        if (prod == 0) {
            return crawl(startX - prod, startY - prod, delX, delY, largest);
        }

        // for (int x = startX, y = startY; x < startX + delX * (consecutive -
        // 1)
        // && y < startY + delY * (consecutive - 1); x += delX, y += delY) {
        // prod *= grid[x][y];
        // }

        for (int x = startX + delX * consecutive, y = startY + delY
                * consecutive; x < grid.length && x >= 0 && y < grid.length
                && y >= 0; x += delX, y += delY) {
            prod *= grid[x][y];
            ...
            prod /= grid[x - delX * (consecutive - 1)][y - delY
                    * (consecutive - 1)];
        }
        return largest;
    }
    ...
    public static int
            getConsecutive(int startX, int startY, int delX, int delY) {
        int prod = 1;
        for (int x = startX, y = startY; x < startX + delX * (consecutive - 1)
                && y < startY + delY * (consecutive - 1); x += delX, y += delY) {
            prod *= grid[x][y];
            if (prod == 0) {
                return x - startX;
                /* return getConsecutive(x + delX, y + delY, delX, delY); */}
        }
        System.out.println(prod);
        return prod;
    }
